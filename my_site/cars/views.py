...
from . import models
from cars.forms import ReviewForm, ReviewFormStars
...
def rental_review_stars(request):
    if request.method == 'POST':
        form = ReviewFormStars(request.POST)
        if form.is_valid():
            form.save()
            return redirect(reverse('cars:thank_you', kwargs={}))
    else:
        form = ReviewFormStars()
        return render(request, 'cars/rental_review_stars.html', context={'form': form})
